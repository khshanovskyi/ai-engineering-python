...
### Local Llama in Docker
17. Open [llama/README.md](llama/README.md) and run a small Llama model locally with Ollama (no API key, no cost) — driving it with the **same** OpenAI Chat Completions and Anthropic clients you already built.

### Gemini
18. Open [gemini/client.py](gemini/interactions/client.py) and implement all TODOs (SDK client)
19. Run [gemini_app.py](gemini/interactions/gemini_app.py) with `gemini_client` and test
20. Open [gemini/custom_client.py](gemini/interactions/custom_client.py) and implement all TODOs (raw HTTP client)
21. In [gemini_app.py](gemini/interactions/gemini_app.py) switch to `gemini_custom_client` and test
